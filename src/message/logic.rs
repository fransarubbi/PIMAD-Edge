...
use crate::grpc_service::domain::GrpcHandle;
use crate::message::domain::*;
use crate::mqtt::domain::{MqttHandle, PayloadTopic};
...
enum InternalMessageCommand {
    ...
    SerializeHelloServer { data: HelloWorld },
...
impl MessageHandle {
    ...
    pub async fn serialize_hello_server(&self, data: HelloWorld) {
                ...
                Some(to_parse) = self.receiver.recv() => {
                    match to_parse {
                        ...
                        InternalEvent::IncomingGrpc(data) => {
                            handle_grpc_message(data, &self.sender).await;
                        }
                        _ => {}
                    }
                }
        ...
        match decoded_msg {
            ...
            HubMessage::FromHubSettings(ref settings) => {
                ...
                if manager.is_active(&network) {
                    ...
                    if tx
                        ...
                        .is_err()
                    {
                        error!("no se pudo enviar el mensaje FromHubSettings");
                    }
                }
            }
            HubMessage::FromHubSettingsAck(ref ack) => {
                ...
                if tx
                    ...
                    .is_err()
                {
                    error!("no se pudo enviar el mensaje FromHubSettingsAck");
                }
            }
...
async fn handle_grpc_message(proto_msg: ToEdge, tx: &mpsc::Sender<MessageServiceResponse>) {
    if let Some(payload) = proto_msg.payload {
        match payload {
            ...
            to_edge::Payload::SettingsAck(setting_ok) => {
                ...
                if tx
                    .send(MessageServiceResponse::FromServer(
                        ServerMessage::FromServerSettingsAck(msg),
                    ))
                    .await
                    .is_err()
                {
